 ...
 *
 * power_distribution_octorotor.c - Coaxial octorotor power distribution code
 */
...
static void powerDistributionLegacy(const control_t *control, motors_thrust_uncapped_t* motorThrustUncapped)
{
  ...
  motorThrustUncapped->motors.m5 = control->thrust - r + p - control->yaw;
  motorThrustUncapped->motors.m6 = control->thrust - r - p + control->yaw;
  motorThrustUncapped->motors.m7 = control->thrust + r - p - control->yaw;
  motorThrustUncapped->motors.m8 = control->thrust + r + p + control->yaw;
}
...
static void powerDistributionForceTorque(const control_t *control, motors_thrust_uncapped_t* motorThrustUncapped) {
  ...
  motorForces[4] = thrustPart - rollPart - pitchPart + yawPart;
  motorForces[5] = thrustPart - rollPart + pitchPart - yawPart;
  motorForces[6] = thrustPart + rollPart + pitchPart + yawPart;
  motorForces[7] = thrustPart + rollPart - pitchPart - yawPart;
